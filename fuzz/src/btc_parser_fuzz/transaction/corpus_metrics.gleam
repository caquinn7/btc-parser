...
fn derive_codes(metrics: Metrics) -> List(String) {
  ...
  [
    ...
    #(metrics.input_count == 252, "I02"),
    #(metrics.input_count == 253, "I03"),
    #(metrics.input_count >= 200, "I04"),
    #(metrics.output_count == 1, "O01"),
    #(metrics.output_count == 252, "O02"),
    #(metrics.output_count == 253, "O03"),
    #(metrics.output_count >= 200, "O04"),
    #(witness.empty_stack_count > 0 && witness.nonempty_stack_count > 0, "W01"),
    #(witness.zero_length_item_count > 0, "W02"),
    #(witness.item_252_count > 0, "W03"),
    #(witness.item_253_count > 0, "W04"),
    #(witness.item_65_535_count > 0, "W05"),
    #(witness.item_65_536_count > 0, "W06"),
    #(witness.stack_252_count > 0, "W07"),
    #(witness.stack_253_count > 0, "W08"),
    #(metrics.witness_size >= metrics.base_size, "W09"),
    #(input.script_sig_252_count > 0, "L01"),
    #(input.script_sig_253_count > 0, "L02"),
    #(output.script_pubkey_252_count > 0, "L03"),
    #(output.script_pubkey_253_count > 0, "L04"),
    #(input.near_policy_script_sig_count > 0, "L05"),
    #(output.near_policy_script_pubkey_count > 0, "L06"),
    ...
    #(output.nonstandard_output_count > 0, "F10"),
    #(output.op_return_nonstandard_output_count > 0, "F11"),
    #(metrics.total_size >= 360_000 && metrics.total_size <= 400_000, "S01"),
    #(metrics.input_count >= 200 && metrics.output_count >= 200, "S02"),
    #(metrics.coinbase_shape && input.max_script_sig_size == 2, "C01"),
    #(metrics.coinbase_shape && input.max_script_sig_size == 100, "C02"),
  ]
