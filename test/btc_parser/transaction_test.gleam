import btc_parser/transaction.{
  BareMultisig, CoinbaseWithMultipleInputs, DecodeFailed, DuplicateInput,
  ...
  NonStandard, NullData, OutputValueOutOfRange, P2PK, P2PKH, P2SH, P2TR, P2WPKH,
  P2WSH, PolicyLimitExceeded, SuperfluousWitnessRecord,
...
import support/bitcoin_wire.{compact_size, get_display_hex}
...
pub fn deserialize_legacy_full_tx_sets_version_and_is_segwit_false_test() {
  let assert Ok(result) = transaction.deserialize_hex(legacy_v1_tx)

  assert transaction.get_version(result) == 1
  assert !transaction.is_segwit(result)
}

pub fn deserialize_legacy_tx_preserves_lock_time_test() {
  let assert Ok(result) = transaction.deserialize_hex(legacy_v1_tx)
  assert transaction.get_lock_time(result) == 939_066
}

pub fn deserialize_segwit_full_tx_sets_version_and_is_segwit_true_test() {
  let assert Ok(result) = transaction.deserialize_hex(segwit_v1_tx)

  assert transaction.get_version(result) == 1
  assert transaction.is_segwit(result)
}

pub fn deserialize_segwit_tx_preserves_lock_time_test() {
  let assert Ok(result) = transaction.deserialize_hex(segwit_v1_tx)
  assert transaction.get_lock_time(result) == 0
}

pub fn deserialize_legacy_v2_preserves_version_2_test() {
  let assert Ok(result) = transaction.deserialize_hex(legacy_v2_tx)
...
pub fn deserialize_version_at_signed_max_as_unsigned_test() {
  let assert Ok(result) =
    transaction.deserialize(build_minimal_tx_with_version(0x7FFFFFFF))
...
pub fn deserialize_version_above_signed_max_as_unsigned_test() {
  let assert Ok(result) =
    transaction.deserialize(build_minimal_tx_with_version(0x80000000))
...
pub fn deserialize_max_unsigned_version_as_unsigned_test() {
  let assert Ok(result) =
    transaction.deserialize(build_minimal_tx_with_version(0xFFFFFFFF))
...
pub fn deserialize_does_not_misclassify_segwit_when_marker_and_flag_are_missing_test() {
  let assert Error(decode_err) = transaction.deserialize(version1)
...
pub fn deserialize_does_not_misclassify_segwit_when_marker_and_flag_are_truncated_test() {
  ...
  let assert Error(decode_err) =
    transaction.deserialize(<<version1:bits, marker:bits>>)
...
pub fn deserialize_returns_invalid_segwit_marker_flag_error_test() {
  ...
  let assert Error(decode_err) =
    transaction.deserialize(<<version1:bits, marker:bits, flag:bits>>)
...
pub fn deserialize_preserves_multiple_inputs_test() {
  ...
  let input1_bytes =
    build_input(outpoint1_txid_bytes, outpoint1_vout, sig1_bytes, seq1)
  let input2_bytes =
    build_input(outpoint2_txid_bytes, outpoint2_vout, sig2_bytes, seq2)
  let input3_bytes =
    build_input(outpoint3_txid_bytes, outpoint3_vout, sig3_bytes, seq3)
...
pub fn deserialize_preserves_multiple_outputs_test() {
  ...
  let output1_bytes = build_output(value1, script1_bytes)
  let output2_bytes = build_output(value2, script2_bytes)
  let output3_bytes = build_output(value3, script3_bytes)
...
pub fn deserialize_segwit_tx_preserves_witness_data_test() {
  // Use the real SegWit transaction constant
  let assert Ok(tx) = transaction.deserialize_hex(segwit_v1_tx)
  ...
  // Verify outputs were preserved correctly.
  let outputs = transaction.get_outputs(tx)
  let assert [_, ..] = outputs

  // Verify witness data was preserved.
...
pub fn round_trip_legacy_tx_wire_bytes_match_original_hex_test() {
  // The bytes produced by serialize must exactly match the original
  ...
  assert transaction.serialize_stripped(tx) == original_bytes
...
fn compare_compute_txid_against_known_vector(
  ...
  let assert Ok(tx) = transaction.deserialize_hex(tx_hex)
...
fn single_input(outpoint_txid: BitArray, outpoint_vout: Int) {
  let input = build_input(outpoint_txid, outpoint_vout, <<>>, 0)
  let lock_time = <<0:little-size(32)>>
...
fn single_input_outpoint(outpoint_txid: BitArray, outpoint_vout: Int) {
  single_input(outpoint_txid, outpoint_vout)
  |> transaction.get_input_outpoint
}
