...
pub fn to_string(perf_result: PerfResult) -> String {
  ...
  metadata_to_string(perf_result.metadata) <> "\n\n" <> table
}
...
fn metadata_to_string(metadata: PerfMetadata) -> String {
  [
...
pub fn to_csv(perf_result: PerfResult) -> String {
  ...
  let rows =
    perf_result.sections
    |> list.flat_map(section_to_csv_rows(perf_result.metadata, _))

  [string.join(headings, with: ","), ..rows]
  |> string.join("\n")
}

fn section_to_csv_rows(
